<?php
require ('mysql2.php'); 
$tbl_name="photo_ladder".$year; // Table name 
...
$id = "('" . implode( "','", $checkbox ) . "');" ;
$sql=$dbc->prepare("UPDATE photo_ladder".$year." SET photog = CONCAT(photog, '$author', '\n') WHERE id IN $id");
$sql->execute();
}
...
ob_start();
require ('header.html');
echo "<h3>Choose photo assignments for ". $author."</h3>"; ?>
...
<tr>
<td colspan="6"><strong>Select the required number of photo assignments</strong> </td>
</tr>
<tr>
    <th>Page #</th>
    <th>Section</th>
...
<?php while($rows=$table_query->fetch(PDO::FETCH_ASSOC)){
?>
<tr>
